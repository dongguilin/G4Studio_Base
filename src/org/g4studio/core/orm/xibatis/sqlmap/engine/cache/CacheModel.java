...
import org.g4studio.core.orm.xibatis.sqlmap.engine.mapping.statement.ExecuteListener;
...
public class CacheModel implements ExecuteListener {
	 ...
	 * so that they can be cached, too.
	 */
	...
	/**
	 * Gets an Iterator containing all flushTriggerStatment objects for this
	 * cache.
	 * 
	 * @return The Iterator
	 */
	...
	/**
	 * ExecuteListener event. This will be called by a MappedStatement for which
	 * this cache is registered as a ExecuteListener. It will be called each
	 * time an executeXXXXXX method is called. In the case of the Cache class,
	 * it is registered in order to flush the cache whenever a certain statement
	 * is executed. (i.e. the flushOnExecute cache policy)
	 * 
	 * @param statement
	...
	/**
	 * Get an object out of the cache. A side effect of this method is that is
	 * may clear the cache if it has not been cleared in the flushInterval.
	 * 
	 * @param key
	...
	/**
	 * Log a cache action. Since this method is pretty heavy weight, it's best
	 * to enclose it with a log.isDebugEnabled() when called.
	 * 
	 * @param action
	 *            String to output
	 * @param addValue
