 ...
 * appropriate Action for each request, create an instance (if necessary), and
 * call the <code>execute</code> method.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * When an <code>Action</code> instance is first created, the controller will
...
public class Action {
	/**
	 * <p>
	 * An instance of <code>TokenProcessor</code> to use for token
	 * functionality.
	 * </p>
	 */
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, ServletRequest request,
		...
		try {
			return execute(mapping, form, (HttpServletRequest) request, (HttpServletResponse) response);
	 ...
	 *            The servlet request we are processing
	 * @param key
	 *            The key specified in the message-resources element for the
	 *            requested bundle.
	 ...
	 * @param errors
	 *            The error messages to save. <code>null</code> or empty
