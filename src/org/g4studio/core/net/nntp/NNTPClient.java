...
import org.g4studio.core.net.io.DotTerminatedMessageReader;
import org.g4studio.core.net.io.DotTerminatedMessageWriter;
...
public class NNTPClient extends NNTP {
	 ...
	 * through an ArticlePointer. The <code> articleId </code> field of the
	 * ArticlePointer cannot always be trusted because some NNTP servers do not
	 ...
	 * instance. The NNTP protocol uses the same stream for issuing commands as
	 ...
	 * contained in the server reply are returned through an ArticlePointer. The
	 * <code> articleId </code> field of the ArticlePointer cannot always be
	 ...
	 * field of the ArticlePointer cannot always be trusted because some NNTP
	 ...
	 * ArticlePointer. The <code> articleId </code> field of the ArticlePointer
	 ...
	 * returned through an ArticlePointer. The <code> articleId </code> field of
	 * the ArticlePointer cannot always be trusted because some NNTP servers do
	...
	/****
	 * Same as <code> selectArticle(null, articleId) </code>. Useful for
	 * retrieving the current article number.
	 ***/
	...
	/**
	 * An overloaded listNewsgroups() command that allows us to specify with a
	 * pattern what groups we want to list. Wraps the LIST ACTIVE command.
	 * <p>
	 * 
	 * @param wildmat
	 ...
	 * message identifier including the enclosing &lt and &gt.
	 * <p>
	 * 
	 * @param query
	...
	public Reader retrieveArticleInfo(int lowArticleNumber, int highArticleNumber) throws IOException {
		return __retrieveArticleInfo(new String(lowArticleNumber + "-" + highArticleNumber));
	}
