 ...
 * is named by the <code>parameter</code> attribute of the corresponding
 ...
 * MappingDispatchAction that has the same signature (other than method name) of
 * the standard Action.execute method. For example, you might combine the
 * methods for managing a subscription into a single MappingDispatchAction class
 ...
 * 
 * <p>
 * <strong>NOTE</strong> - Unlike DispatchAction, mapping characteristics may
 * differ between the various handlers, so you can combine actions in the same
 * class that, for example, differ in their use of forms or validation. Also, a
 * request parameter, which would be visible to the application user, is not
 * required to enable selection of the handler method.
 * </p>
 * 
...
public class MappingDispatchAction extends DispatchAction {
	 ...
	 * the ActionMapping. Subclasses of <code>MappingDispatchAction</code>
